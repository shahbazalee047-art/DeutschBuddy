...
export default function ListeningTask({ content, onComplete }) {
  ...
  return (
    <div className="fade-in reading-body">
      ...
      {clip.title && <div className=" p-4 mb-4 border border-gold/20" style={{ background: 'rgba(232,163,61,0.05)' }}><div className="flex items-center gap-3"><SpeakerButton text={clip.title} size="lg" /><div><p className="text-sm font-semibold text-text-body">{clip.title}</p><p className="text-[11px] text-text-muted">{clip.source}</p></div></div>{clip.text && <p className="text-[12px] text-text-muted mt-2 italic">{clip.text}</p>}</div>}
      <div className="space-y-3">{qs.map((q, i) => (<div key={i} className="paper-card p-4"><p className="text-[14px] font-medium text-text-body mb-3"><span className="font-bold mr-1 text-gold">{i + 1}.</span>{q.question}</p><div className="space-y-2">{q.options.map((o, j) => { let s = 'bg-bg-secondary border-border hover:border-gold/30'; if (sub) { if (j === q.correct) s = 'bg-success/10 border-success text-success'; else if (ans[i] === j) s = 'bg-error/10 border-error text-error'; else s = 'bg-bg-secondary/50 border-border opacity-40'; } else if (ans[i] === j) s = 'bg-bg-secondary border-gold'; return (              <button key={j} onClick={() => !sub && setAns(p => ({ ...p, [i]: j }))} disabled={sub} className={`w-full text-left border text-sm transition-all active:scale-[0.98] rounded-[var(--radius-exercise)] quiz-option ${s}`}>
                <span className="font-medium text-text-muted">{String.fromCharCode(65 + j)}.</span> <span className="text-text-body">{o}</span>
              </button>); })}</div></div>))}</div>
