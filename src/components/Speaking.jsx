...
export default function Speaking({ content, onComplete }) {
  ...
  return (
    <div className="fade-in">
      <h3 className="font-bold text-zinc-100 text-lg mb-5 flex items-center gap-2"><IconMic className="w-5 h-5 text-cyan-400" /> Speaking Practice</h3>
      <div className="glass-card p-5 mb-4">
        <div className="rounded-2xl p-4 mb-4 border border-lime-500/20" style={{ background: 'rgba(163, 230, 53, 0.05)' }}>
          <p className="text-[11px] font-bold text-lime-400 mb-1 uppercase flex items-center gap-1" style={{ letterSpacing: '0.5px' }}><IconMic className="w-3.5 h-3.5" /> Say this:</p>
        ...
        {tips.length > 0 && <div className="mt-3 p-3 rounded-xl border border-cyan-500/20" style={{ background: 'rgba(6, 182, 212, 0.05)' }}><p className="text-[11px] font-bold text-cyan-400 mb-1 flex items-center gap-1"><IconLightbulb className="w-3.5 h-3.5" /> Tips:</p><ul className="text-[12px] text-zinc-300 space-y-0.5">{tips.map((t, i) => <li key={i}>• {t}</li>)}</ul></div>}
      </div>
