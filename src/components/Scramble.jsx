...
export default function Scramble({ content, onComplete }) {
  ...
  function submit() { const ok = input.trim().toLowerCase() === answer.toLowerCase(); if (ok) setScore(p => p + 1); setShow(true); setTimeout(() => { if (isLast) onComplete(); else { setIdx(p => p + 1); setInput(''); setShow(false); } }, 1200); }
  return (
    <div className="fade-in reading-body">
      ...
      <div className="paper-card p-6 mb-4 text-center">
        ...
        <div className="flex items-center justify-center gap-3 mb-3"><input type="text" value={input} onChange={e => setInput(e.target.value)} onKeyDown={e => e.key === 'Enter' && !show && submit()} disabled={show} placeholder="Type the word..." className="w-56 paper-input text-center text-lg font-medium fill-blank-input" /><SpeakerButton text={answer} size="md" /></div>
        {!show && <button onClick={submit} className="btn-primary px-6 active:scale-95">Check</button>}
      </div>
      {show && <div className={`text-center p-3  text-sm font-semibold text-text-on-dark`} style={{ background: input.trim().toLowerCase() === answer.toLowerCase() ? 'var(--success)' : 'var(--gold-light)' }}>{input.trim().toLowerCase() === answer.toLowerCase() ? <span className="flex items-center justify-center gap-1">Richtig! <IconSparkles className="w-4 h-4" /></span> : <span className="flex items-center justify-center gap-1">Answer: "{answer}" <IconHeart className="w-4 h-4" /></span>}</div>}
    </div>
  );
}
