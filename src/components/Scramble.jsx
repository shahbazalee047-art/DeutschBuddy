...
export default function Scramble({ content, onComplete }) {
  ...
  return (
    <div className="fade-in">
      <div className="flex justify-between items-center mb-5"><h3 className="font-bold text-cream-100 text-lg flex items-center gap-2"><IconShuffle className="w-5 h-5 text-sage-400" /> Unscramble</h3><span className="text-sm font-bold text-sage-400">{score}/{words.length}</span></div>
      ...
      <div className="glass-card p-6 mb-4 text-center">
        <p className="text-[11px] text-cream-500 mb-4 uppercase font-medium" style={{ letterSpacing: '0.5px' }}>Unscramble the German word:</p>
        <div className="flex justify-center gap-2 mb-5">{shuffled.map((l, i) => (<span key={i} className="w-12 h-12 flex items-center justify-center bg-forest-800 border border-border rounded-xl text-lg font-bold text-cream-200">{l}</span>))}</div>
