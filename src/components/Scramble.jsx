...
export default function Scramble({ content, onComplete }) {
  ...
  return (
    <div className="fade-in">
      <div className="flex justify-between items-center mb-5"><h3 className="font-bold text-slate-200 text-lg">🔀 Unscramble</h3><span className="text-sm font-bold" style={{ color: '#FFCC00' }}>{score}/{words.length}</span></div>
      <div className="w-full h-2 bg-slate-700 rounded-full overflow-hidden mb-5"><div className="h-full rounded-full transition-all duration-300" style={{ width: `${((idx + 1) / words.length) * 100}%`, background: 'linear-gradient(to right, #FFCC00, #ffe066)' }} /></div>
      <div className="bg-slate-800 border border-slate-700/50 rounded-2xl p-6 mb-4 text-center">
        ...
        <div className="flex items-center justify-center gap-2 mb-3"><input type="text" value={input} onChange={e => setInput(e.target.value)} onKeyDown={e => e.key === 'Enter' && !show && submit()} disabled={show} placeholder="Type the word..." className="w-56 px-4 py-2.5 bg-slate-900/50 border border-slate-700 rounded-xl text-center text-base font-medium text-slate-200 placeholder-slate-600 focus:outline-none focus:ring-2 focus:ring-[#FFCC00] transition" /><SpeakerButton text={w.answer} size="md" /></div>
        {!show && <button onClick={submit} className="px-5 py-2 text-black rounded-xl text-sm font-semibold hover:scale-[1.02] transition" style={{ background: '#FFCC00' }}>Check</button>}
      </div>
      {show && <div className={`text-center p-3 rounded-xl text-sm font-medium ${input.trim().toLowerCase() === w.answer.toLowerCase() ? 'text-black' : 'bg-red-500/10 text-red-400 border border-red-500/20'}`} style={input.trim().toLowerCase() === w.answer.toLowerCase() ? { background: '#FFCC00' } : {}}>{input.trim().toLowerCase() === w.answer.toLowerCase() ? 'Richtig! 🎉' : `Answer: "${w.answer}" 💪`}</div>}
    </div>
  );
}
