...
export default function Scramble({ content, onComplete }) {
  ...
  return (
    <div className="fade-in">
      <div className="flex justify-between items-center mb-5"><h3 className="font-bold text-[#1A1A2E] text-lg">🔀 Unscramble</h3><span className="text-sm font-bold" style={{ color: '#B8860B' }}>{score}/{words.length}</span></div>
      ...
      <div className="paper-card p-6 mb-4 text-center">
        <p className="text-[11px] text-[#8A8A9A] mb-4 uppercase font-medium" style={{ letterSpacing: '0.5px' }}>Unscramble the German word:</p>
        <div className="flex justify-center gap-2 mb-5">{shuffled.map((l, i) => (<span key={i} className="w-12 h-12 flex items-center justify-center bg-[#F5EFE6] border border-[#E8E0D4] rounded-xl text-lg font-bold text-[#1A1A2E]">{l}</span>))}</div>
      ...
      {show && <div className={`text-center p-3 rounded-2xl text-sm font-semibold text-white`} style={{ background: input.trim().toLowerCase() === w.answer.toLowerCase() ? '#4CAF50' : '#FF9800' }}>{input.trim().toLowerCase() === w.answer.toLowerCase() ? 'Richtig! 🎉' : `Answer: "${w.answer}" 💪`}</div>}
    </div>
  );
}
