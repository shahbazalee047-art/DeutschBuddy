...
export default function Fun({ content, onComplete }) {
  ...
  return (
    <div className="glass-card p-4 slide-up">
      <div className="flex items-center gap-2 mb-3"><IconSparkles className="w-5 h-5 text-lime-400" /><h4 className="text-sm font-bold text-zinc-200">Bonus Content</h4></div>
      ...
      <button onClick={onComplete} className="mt-3 w-full py-2.5 text-[13px] font-medium text-zinc-400 bg-zinc-800 border border-zinc-700 rounded-xl hover:bg-zinc-700 transition active:scale-95 flex items-center justify-center gap-1.5"><IconCheck className="w-4 h-4" /> Done! Finish Day</button>
    </div>
  );
}
