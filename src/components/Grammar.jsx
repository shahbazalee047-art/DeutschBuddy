  ...
  return (
    <div className="fade-in">
      ...
      <div className="glass-card p-5">
        ...
        {content.examples?.length > 0 && <div className="mb-4"><button onClick={() => setShowEx(!showEx)} className="text-sm font-bold text-cream-300 mb-2 flex items-center gap-1.5">{showEx ? '▼' : '▶'} Examples</button>{showEx && <div className="space-y-2 ml-4">{content.examples.map((ex, i) => (<div key={i} className="border border-border rounded-xl p-3 bg-forest-800"><div className="flex items-center gap-2"><p className="font-medium text-cream-200 flex-1">{ex.german}</p><SpeakerButton text={ex.german} size="sm" /></div><p className="text-[12px] text-cream-400">{ex.english}</p></div>))}</div>}</div>}
