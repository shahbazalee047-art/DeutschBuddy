...
export default function Flashcards({ content, onComplete }) {
  ...
  return (
    <div className="fade-in">
      <div className="flex justify-between items-center mb-5"><h3 className="font-bold text-zinc-100 text-lg">🃏 Flashcards</h3><span className="text-sm text-zinc-400">{idx + 1}/{cards.length}</span></div>
      <div className="flex justify-center mb-4">{cards.map((_, i) => (<div key={i} className={`w-2.5 h-2.5 rounded-full mx-0.5 transition ${i === idx ? 'bg-lime-400' : done.includes(i) ? 'bg-success' : 'bg-zinc-700'}`} />))}</div>
      <div onClick={() => setFlipped(!flipped)} className="relative w-full max-w-md mx-auto cursor-pointer" style={{ perspective: '1000px' }}>
        <div className="w-full min-h-[240px] rounded-3xl transition-all duration-500" style={{ transformStyle: 'preserve-3d', transform: flipped ? 'rotateY(180deg)' : 'rotateY(0deg)' }}>
          <div className="absolute inset-0 w-full min-h-[240px] rounded-3xl flex flex-col items-center justify-center p-6" style={{ background: 'linear-gradient(135deg, #A3E635, #06B6D4)', backfaceVisibility: 'hidden', boxShadow: '0 8px 32px rgba(163, 230, 53, 0.3)' }}>
          ...
          <div className="absolute inset-0 w-full min-h-[240px] rounded-3xl glass-card flex flex-col items-center justify-center p-6" style={{ backfaceVisibility: 'hidden', transform: 'rotateY(180deg)' }}>
            <div className="flex items-center gap-2 mb-2"><div className="text-xl font-bold text-zinc-100">{card.front}</div><SpeakerButton text={card.front} size="sm" /></div>
            <div className="text-base mb-2 font-medium text-cyan-400">{card.back}</div>
      ...
      <div className="flex justify-center gap-3 mt-5">
        <button onClick={() => { if (idx > 0) { setFlipped(false); setIdx(p => p - 1); } }} disabled={idx === 0} className="px-6 py-2.5 rounded-xl border border-zinc-700 text-zinc-400 hover:bg-zinc-800 disabled:opacity-40 transition text-sm active:scale-95">← Prev</button>
