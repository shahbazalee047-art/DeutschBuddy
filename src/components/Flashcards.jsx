...
export default function Flashcards({ content, onComplete }) {
  ...
  return (
    <div className="fade-in">
      ...
      <div className="flex justify-center mb-4">{cards.map((_, i) => (<div key={i} className={`w-2 h-2 rounded-full mx-0.5 transition ${i === idx ? 'bg-lime-400' : done.includes(i) ? 'bg-cyan-400' : 'bg-slate-700'}`} />))}</div>
      <div onClick={() => setFlipped(!flipped)} className="relative w-full max-w-md mx-auto cursor-pointer" style={{ perspective: '1000px' }}>
        <div className="w-full min-h-[220px] rounded-2xl transition-all duration-500" style={{ transformStyle: 'preserve-3d', transform: flipped ? 'rotateY(180deg)' : 'rotateY(0deg)' }}>
          <div className="absolute inset-0 w-full min-h-[220px] rounded-2xl bg-gradient-to-br from-lime-400 to-lime-500 flex flex-col items-center justify-center text-slate-900 p-6" style={{ backfaceVisibility: 'hidden' }}>
            ...
            <div className="text-sm text-slate-900/60 mt-4">Click to flip</div>
          </div>
      ...
      <div className="flex justify-center gap-3 mt-5"><button onClick={() => { if (idx > 0) { setFlipped(false); setIdx(p => p - 1); } }} disabled={idx === 0} className="px-5 py-2 rounded-xl border border-slate-700 text-slate-400 hover:bg-slate-800 disabled:opacity-40 transition text-sm">← Prev</button><button onClick={next} className="px-5 py-2 rounded-xl bg-lime-400 text-slate-900 font-semibold hover:bg-lime-300 transition text-sm">{isLast ? '✓ Complete' : 'Next →'}</button></div>
    </div>
  );
}
