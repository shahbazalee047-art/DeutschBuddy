...
export default function Quiz({ content, onComplete }) {
  ...
  return (
    <div className="fade-in">
      <div className="flex justify-between items-center mb-5"><h3 className="font-bold text-zinc-100 text-lg flex items-center gap-2"><IconHelpCircle className="w-5 h-5 text-lime-400" /> Quiz</h3><span className="text-sm font-bold text-lime-400">{score}/{qs.length}</span></div>
        ...
        <div className="space-y-3">
          {q.options.map((opt, i) => { let s = 'bg-zinc-800 border-zinc-700 hover:border-lime-500/30'; if (show) { if (i === q.correct) s = 'bg-green-500/10 border-success text-green-400'; else if (i === sel) s = 'bg-error/10 border-error text-error'; else s = 'bg-zinc-800/50 border-zinc-700 opacity-40'; } else if (i === sel) s = 'bg-zinc-700 border-lime-500'; return (
            <button key={i} onClick={() => pick(i)} disabled={show} className={`w-full text-left p-4 rounded-2xl border transition-all active:scale-[0.98] ${s}`}>
              <div className="flex items-center gap-3"><span className="w-8 h-8 rounded-xl bg-zinc-700 flex items-center justify-center text-xs font-bold text-zinc-400">{String.fromCharCode(65 + i)}</span><span className="text-[14px] text-zinc-300 font-medium">{opt}</span>{show && i === q.correct && <IconCheck className="w-5 h-5 text-success ml-auto" />}{show && i === sel && i !== q.correct && <IconX className="w-5 h-5 text-error ml-auto" />}</div>
            </button>
          );})}
        </div>
      </div>
      {show && <div className={`text-center p-3 rounded-2xl text-sm font-semibold text-zinc-900`} style={{ background: sel === q.correct ? '#22C55E' : '#F59E0B' }}>{sel === q.correct ? <span className="flex items-center justify-center gap-1">Richtig! <IconSparkles className="w-4 h-4" /></span> : 'Fast richtig! Almost there!'}</div>}
    </div>
  );
}
