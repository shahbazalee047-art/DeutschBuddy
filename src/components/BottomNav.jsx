...
export default function BottomNav({ activeView, onViewChange }) {
  const items = [
    ...
    { id: 'progress-statistics', label: 'Progress', icon: IconChart },
  ...
  const isProgressActive = activeView === 'progress-statistics' || activeView === 'progress-skills' || activeView === 'progress-calendar';
      ...
      <div className="grid grid-cols-5 h-16">
        {items.map(item => (
          <button key={item.id} onClick={() => onViewChange(item.id)}
            ...
            {(item.id === 'progress-statistics' ? isProgressActive : activeView === item.id) && <div className="w-1 h-1 rounded-full bg-lime-400 mt-0.5" />}
          </button>
        ))}
      </div>
    </nav>
