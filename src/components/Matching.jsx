...
export default function Matching({ content, onComplete }) {
  ...
  return (
    <div className="fade-in">
      <div className="flex justify-between items-center mb-5"><h3 className="font-bold text-zinc-100 text-lg flex items-center gap-2"><IconLink className="w-5 h-5 text-lime-400" /> Matching</h3><span className="text-sm text-zinc-400">{matched.length}/{pairs.length}</span></div>
      <div className="grid grid-cols-2 gap-4">
        <div className="space-y-2"><h4 className="text-[11px] font-bold text-cyan-400 mb-2 text-center uppercase flex items-center justify-center gap-1" style={{ letterSpacing: '0.5px' }}><IconFlag className="w-3.5 h-3.5" /> Deutsch</h4>
          {pairs.map((p, i) => (<button key={i} onClick={() => clickDE(i)} disabled={matched.includes(i)} className={`w-full p-3 rounded-xl border text-left text-sm transition-all active:scale-95 ${matched.includes(i) ? 'bg-lime-500/10 border-lime-500/20 text-lime-400' : sel === i ? 'bg-lime-500/10 border-lime-500 text-zinc-200' : 'glass-card text-zinc-300 hover:border-lime-500/30'}`}>
            <div className="flex items-center justify-between"><span className="font-medium">{p.german}</span><div className="flex items-center gap-1">{matched.includes(i) && <IconCheck className="w-4 h-4 text-lime-400" />}<SpeakerButton text={p.german} size="sm" /></div></div></button>))}
        </div>
        <div className="space-y-2"><h4 className="text-[11px] font-bold text-zinc-500 mb-2 text-center uppercase flex items-center justify-center gap-1" style={{ letterSpacing: '0.5px' }}><IconMessageCircle className="w-3.5 h-3.5" /> English</h4>
      ...
      {matched.length === pairs.length && <div className="text-center mt-4 p-3 rounded-2xl font-bold text-sm text-zinc-900 flex items-center justify-center gap-1.5" style={{ background: 'linear-gradient(135deg, #A3E635, #06B6D4)' }}><IconSparkles className="w-4 h-4" /> All matched! Ausgezeichnet!</div>}
    </div>
  );
}
