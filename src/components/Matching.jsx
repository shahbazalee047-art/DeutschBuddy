    ...
    <div className="fade-in">
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="space-y-2"><h4 className="text-[11px] font-bold text-gold mb-2 text-center uppercase flex items-center justify-center gap-1" style={{ letterSpacing: '0.5px' }}><IconFlag className="w-3.5 h-3.5" /> Deutsch</h4>
          {pairs.map((p, i) => (<button key={i} onClick={() => clickDE(i)} disabled={matched.includes(i)} className={`w-full p-3  border text-left text-sm transition-all active:scale-95 ${matched.includes(i) ? 'bg-gold/10 border-gold/20 text-gold' : sel === i ? 'bg-gold/10 border-gold text-text-body' : 'paper-card text-text-body hover:border-gold/30'}`}>
        ...
        <div className="space-y-2"><h4 className="text-[11px] font-bold text-text-muted mb-2 text-center uppercase flex items-center justify-center gap-1" style={{ letterSpacing: '0.5px' }}><IconMessageCircle className="w-3.5 h-3.5" /> English</h4>
          {shuffled.map((p, i) => { const isMatched = matched.some(m => pairs[m].english === p.english); return (
            <button key={i} onClick={() => clickEN(p)} disabled={isMatched} className={`w-full p-3  border text-left text-sm transition-all active:scale-95 ${isMatched ? 'bg-gold/10 border-gold/20 text-gold' : 'paper-card text-text-body hover:border-gold/30'}`}><span className="font-medium">{p.english}</span></button>
          );})}
        </div>
      </div>
