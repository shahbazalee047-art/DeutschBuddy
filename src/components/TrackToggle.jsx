import { IconTurtle, IconBolt } from './Icons';
  ...
  return (
    <div className="flex items-center gap-1.5 p-1.5 rounded-2xl border border-zinc-700/50" style={{ background: '#1E1E24' }}>
      ...
      <button onClick={() => onToggle('standard')}
        ...
        <IconTurtle className="w-4 h-4 inline-block align-text-bottom mr-1" /> Standard
      </button>
      <button onClick={() => onToggle('fast')}
        ...
        <IconBolt className="w-4 h-4 inline-block align-text-bottom mr-1" /> Fast
      </button>
    </div>
  );
}
