...
export default function FillBlank({ content, onComplete }) {
  ...
  return (
    <div className="fade-in">
      <div className="flex justify-between items-center mb-5"><h3 className="font-bold text-cream-100 text-lg flex items-center gap-2"><IconEdit className="w-5 h-5 text-sage-400" /> Fill in the Blank</h3><span className="text-sm font-bold text-sage-400">{score}/{sents.length}</span></div>
      ...
      <div className="glass-card p-6 mb-4 text-center">
        <p className="text-[18px] text-cream-200 mb-5">{s.text.split('___').map((p, i) => <span key={i}>{p}{i < s.text.split('___').length - 1 && <span className="inline-block min-w-[100px] mx-1 border-b-2 text-center font-bold" style={{ borderColor: '#7FB069', color: '#7FB069' }}>{show ? s.answer : '___'}</span>}</span>)}</p>
