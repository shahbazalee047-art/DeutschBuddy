...
export default function FillBlank({ content, onComplete }) {
  ...
  return (
    <div className="fade-in">
      <div className="flex justify-between items-center mb-5"><h3 className="font-bold text-zinc-100 text-lg">✏️ Fill in the Blank</h3><span className="text-sm font-bold text-lime-400">{score}/{sents.length}</span></div>
      ...
      {show && <div className={`text-center p-3 rounded-2xl text-sm font-semibold text-zinc-900`} style={{ background: ans.trim().toLowerCase() === s.answer.toLowerCase() ? '#22C55E' : '#F59E0B' }}>{ans.trim().toLowerCase() === s.answer.toLowerCase() ? 'Richtig! 🎉' : `Answer: "${s.answer}" 💪`}</div>}
    </div>
  );
}
