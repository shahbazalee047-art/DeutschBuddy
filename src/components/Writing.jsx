...
export default function Writing({ content, onComplete }) {
  ...
  return (
    <div className="fade-in">
      <h3 className="font-bold text-zinc-100 text-lg mb-5">✍️ Writing Exercise</h3>
      <div className="glass-card p-5">
        <div className="rounded-2xl p-4 mb-4 border border-lime-500/20" style={{ background: 'rgba(163, 230, 53, 0.05)' }}><p className="text-[11px] font-bold text-lime-400 mb-1 uppercase" style={{ letterSpacing: '0.5px' }}>📝 Task:</p><p className="text-[14px] text-zinc-300">{content.prompt}</p></div>
      ...
      {done && <div className="text-center p-3 rounded-2xl text-sm font-semibold text-zinc-900 mt-4" style={{ background: 'linear-gradient(135deg, #A3E635, #06B6D4)' }}>✍️ Gut geschrieben! Keep practicing!</div>}
    </div>
  );
}
